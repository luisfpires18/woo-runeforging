...
import type { SettlementState } from '../api/types.ts';
 ...
 *
 * Settlement identity, its state and its stores share a single
...
export function AppShell({
  ...
  readonly state: SettlementState | null;
  readonly offline: boolean;
  readonly children: ReactNode;
}) {
  const { path } = useRouter();
  const { advanceTime } = useSettlementState();

  return (
    <div className="shell">
      {/* The pass above the outpost. Decorative depth, never information. */}
      <div
      ...
      {offline && (
        <div className="offline" role="status">
          You are offline. What you can see is the last word from the outpost.
        </div>
      )}
      ...
      <header className="hud">
        <div className="hud__inner">
          <div className="hud__identity">
            {state !== null && (
              <Art
                assetKey={state.settlement.crestKey}
                className="hud__crest"
                factionFallback={false}
              />
            )}
            {/* One name, said once. The line beneath it says where and what,
                not the same words again. */}
            <div className="hud__names">
              <span className="hud__name">{state?.settlement.name ?? 'Settlement'}</span>
              <span className="hud__where">
                {state?.settlement.kingdom}
